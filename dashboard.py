...
# Create a DataFrame
    ...
    base = alt.Chart(filtered_glc_df).mark_line(color=color_palette["Velocity_Category"]["Under 8ft/s"]).encode(
        x=alt.X(
    ...
    closed_gates = filtered_glc_df[['gate_min_datetime', 'gate_max_datetime', 'gate_status']].drop_duplicates().reset_index(drop=True)
    ...
    yrule = alt.Chart(filtered_glc_df).mark_rule(color = "red", strokeDash=[12, 6], size=1.5).encode(
            y=alt.datum(8)
    ).properties(
        width=300,
        height=300
    ...
    rules = alt.Chart(filtered_glc_df).transform_calculate(
    ...
    vel_text = alt.Chart(filtered_glc_df).mark_text(align='right').encode(
    ...
    vel_duration_text = alt.Chart(filtered_glc_df).mark_text(align='right').encode(
    ...
    gate_duration_text = alt.Chart(filtered_glc_df).mark_text(align='right').encode(
